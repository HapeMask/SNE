...
P = tt.exp(-pdist_X / (2 * sigma**2)) / tt.exp(-pdist_X / (2 * sigma**2)).sum(1)[:, np.newaxis]
Q = tt.exp(-pdist_Y) / tt.exp(-pdist_Y).sum(1)[:, np.newaxis]
KL = tt.where(abs(P) > 1e-8, P * tt.log(P / Q), 0).sum(1)
C = KL.sum()
LogPerplexity = -tt.where(abs(P) > 1e-8, P*tt.log2(P), 0).sum(1)
